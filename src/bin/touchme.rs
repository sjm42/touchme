...
fn main() -> anyhow::Result<()> {
    ...
    opts.start_pgm(me);
    let re_date = Regex::new(RE_YYYYMMDD)?;
    debug!("opts: {opts:?}");

    for a in &opts.args {
        let md = fs::metadata(a)?;
        if md.is_file() {
            if let Some(ts) = detect_date_filename(path::Path::new(a), &re_date)
                && !opts.dry_run
            {
            ...
            debug!("Found {} files:\n{:?}", files.len(), files);
            for f in files {
                if let Some(ts) = detect_date_filename(&f, &re_date)
                    && !opts.dry_run
                {
...
fn detect_date_filename(file_path: &path::Path, re_date: &Regex) -> Option<time::SystemTime> {
    ...
    if let Some(caps) = re_date.captures(&filename) {
        let (year, month, day) = (
            caps[1].parse::<i32>().ok()?,
            caps[2].parse::<u32>().ok()?,
            caps[3].parse::<u32>().ok()?,
